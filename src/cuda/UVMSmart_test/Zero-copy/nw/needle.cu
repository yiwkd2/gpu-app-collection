...
void runTest( int argc, char** argv) 
{
    ...
    cudaHostAlloc(&referrence, sizeof(int)*size, cudaHostAllocMapped);
    cudaHostGetDevicePointer(&d_referrence, referrence, 0);
    cudaHostAlloc(&itemsets, sizeof(int)*size, cudaHostAllocMapped);
    cudaHostGetDevicePointer(&d_itemsets, itemsets, 0);
    ...
    /*
    printf("kernel execution is completed. dump memory, (referrence) addr: %p, size: %x, "
            "(itemsets) addr: %p, size: %x\n",
